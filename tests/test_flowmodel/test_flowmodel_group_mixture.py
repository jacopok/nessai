...
def test_set_coordinate_bridge_resets_canon_seen(wrapper, rng):
    x = torch.cat([points_in_element(k, 50, rng) for k in range(GROUP_SIZE)])
    wrapper.update_base_standardisation(x)
    assert bool(wrapper._canon_seen)
    wrapper.set_coordinate_bridge(_logit_bridge())
    assert not bool(wrapper._canon_seen)
...
def test_update_base_standardisation_pools_all_branches(base_flow):
    """The shared mean/std is a direct pool of every claimed point across
    every branch -- not tracked or capped per branch -- since every group
    element is a symmetric copy of the same underlying mode."""
    w = DiscreteGroupMixtureFlowWrapper(
        base_flow=base_flow,
        num_features=2,
        group_action_fn=shift_group_action,
    ...
    rng = np.random.default_rng(0)
    branches = [_branch_data(rng, k, 400, 0.2) for k in range(GROUP_SIZE)]
    x_train = torch.tensor(np.concatenate(branches), dtype=torch.float32)
    w.update_base_standardisation(x_train)

    pooled_y = np.concatenate(branches)[:, 1]
    # ddof=1 to match torch's ``.std()`` default (unbiased).
    assert float(w._canon_std[1]) == pytest.approx(
        pooled_y.std(ddof=1), rel=1e-5
    )
    assert float(w._canon_mean[1]) == pytest.approx(pooled_y.mean(), abs=1e-3)


def test_update_base_standardisation_ema_across_rounds(base_flow):
    """A second round exponentially averages into the first, rather than
    ...
    assert float(w._canon_mean[1]) == pytest.approx(0.0, abs=0.05)
    ...
    # moved towards 5.0 by exactly one EMA step, not snapped to it.
    assert float(w._canon_mean[1]) == pytest.approx(beta * 5.0, abs=0.1)


def test_update_base_standardisation_skips_when_too_few_claimed(base_flow):
    w = DiscreteGroupMixtureFlowWrapper(
        base_flow=base_flow,
        num_features=2,
        group_action_fn=shift_group_action,
    ...
    x = points_in_element(0, 4, np.random.default_rng(0))  # < _min_std_count
    w.update_base_standardisation(x)
    assert not bool(w._canon_seen)
...
def test_reflect_standardisation_pinned_and_samples_in_domain(base_flow):
    ...
    assert w._canon_mean[0].abs() == 0.0
    assert w._canon_std[0] > 0
    # y untouched -> ordinary mean/std
    assert w._canon_mean[1].abs() < 0.2
...
def test_reset_model_carries_over_branch_state():
    ...
    with torch.no_grad():
        ...
        old._canon_mean.copy_(torch.full((d,), 3.5))
        old._canon_std.copy_(torch.full((d,), 2.5))
